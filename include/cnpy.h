...
namespace cnpy {
	...
	template<typename T> void npz_save(const std::string& zipname, const std::string& fname, const T* data, const std::vector<size_t>& shape, const std::string& mode = "w")
	{
		//first, append a .npy to the fname
		std::string full_fname = fname + ".npy";
		...
		local_header.reserve(30 + full_fname.size());
		...
		local_header += (uint16_t)full_fname.size(); //fname length
		local_header += (uint16_t)0; //extra field length
		local_header += full_fname;

		//build global header
		global_header.reserve(global_header.size() + 46 + full_fname.size());
	...
	template<typename T> std::vector<char> create_npy_header(const std::vector<size_t>& shape) {
		...
		dict.reserve(128); // Reserve some space to avoid multiple reallocations
		dict += "{'descr': '";
		dict += BigEndianTest();
		...
		header.reserve(10 + dict.size()); // Reserve space for the header
		header += (char)0x93;
		header += "NUMPY";
